...
FROM bedrift INNER JOIN oppdrag USING (orgNr)
    LEFT JOIN historikk USING (oppdragsNr) WHERE oppdragsNr IS NOT NULL
ORDER BY oppdragsNr;
...
SELECT idKandidat, CONCAT(fornavn, " ", etternavn) AS "Navn", idKvalifikasjon, beskrivelse
FROM kvalifikasjon
    INNER JOIN kandidatkvalifikasjon USING (idKvalifikasjon)
    INNER JOIN kandidat USING (idKandidat);

-- 4. Som oppgave 3), men få med de kandidatene som ikke er registrert med kvalifikasjoner.
SELECT idKandidat, CONCAT(fornavn, " ", etternavn) AS "Navn", idKvalifikasjon, beskrivelse
FROM kvalifikasjon
    INNER JOIN kandidatkvalifikasjon USING (idKvalifikasjon)
    RIGHT JOIN kandidat USING (idKandidat);

-- 5. Skriv ut jobbhistorikken til en bestemt vikar, gitt kandidatnr. Vikarnavn, sluttdato, oppdragsnr og bedriftsnavn skal med.
SELECT CONCAT(fornavn, " ", etternavn) AS "Navn", historikk.sluttDato, oppdragsNr, navn
FROM historikk
    INNER JOIN oppdrag USING(oppdragsNr)
    INNER JOIN kandidat USING (idKandidat)
    INNER JOIN bedrift USING (orgNr)
WHERE idKandidat = 1;